...
partial class UT_RpcServer
{
    ...
    {
        TestUtilOpenWallet();
        var assetId = NativeContract.EpicChain.Hash;
    ...
    {
        _rpcServer.wallet = null;
        var exception = Assert.ThrowsException<RpcException>(() => _rpcServer.GetWalletBalance(new JArray(NativeContract.EpicChain.Hash.ToString())), "Should throw RpcException for no opened wallet");
        Assert.AreEqual(exception.HResult, RpcError.NoOpenedWallet.Code);
    }
