...
//
// UT_IOHelper.cs is a component of the EpicChain Labs project, founded by xmoohad. This file is
    ...
    public class UT_IOHelper
    {
        ...
        public void TestAsSerializableGeneric()
        {
            byte[] caseArray = new byte[] { 0x00,0x00,0x00,0x00,0x00,
                                            ...
                                            0x00,0x00,0x00,0x00,0x00,
                                            0x00,0x00,0x00,0x00,0x00 };
            UInt160 result = EpicChain.IO.Helper.AsSerializable<UInt160>(caseArray);
        ...
        public void TestReadFixedBytes()
        {
            ...
            {
                byte[] result = EpicChain.IO.Helper.ReadFixedBytes(reader, 3);
            ...
            using (BinaryReader reader = new(new MemoryStream(data), Encoding.UTF8, false))
            {
                byte[] result = EpicChain.IO.Helper.ReadFixedBytes(reader, 4);
            ...
            using (BinaryReader reader = new(new MemoryStream(data), Encoding.UTF8, false))
            {
                Assert.ThrowsException<FormatException>(() => EpicChain.IO.Helper.ReadFixedBytes(reader, 5));
                Assert.AreEqual(4, reader.BaseStream.Position);
            }
        }
        ...
        public void TestNullableArray()
        {
            ...
            {
                EpicChain.IO.Helper.WriteNullableArray(writter, caseArray);
                data = stream.ToArray();
            }
            ...
            var read = EpicChain.IO.Helper.ReadNullableArray<UInt160>(ref reader);
            ...
            for (int i = 0; i < 2; i++)
            {
                if (i == 0)
                {
                    byte[] caseArray = new byte[] { 0x00,0x00,0x00,0x00,0x00,
                                                    ...
                                                    0x00,0x00,0x00,0x00,0x00,
                                                    0x00,0x00,0x00,0x00,0x00 };
                    ISerializable result = EpicChain.IO.Helper.AsSerializable(caseArray, typeof(UInt160));
                    Assert.AreEqual(UInt160.Zero, result);
                }
                else
                {
                    Action action = () => EpicChain.IO.Helper.AsSerializable(Array.Empty<byte>(), typeof(double));
        ...
        public void TestCompression()
        {
            ...
            var byteArray = EpicChain.IO.Helper.CompressLz4(data);
            var result = EpicChain.IO.Helper.DecompressLz4(byteArray.Span, byte.MaxValue);
            ...
            byteArray = EpicChain.IO.Helper.CompressLz4(data);
            result = EpicChain.IO.Helper.DecompressLz4(byteArray.Span, byte.MaxValue);
            ...
            Assert.ThrowsException<FormatException>(() => EpicChain.IO.Helper.DecompressLz4(byteArray.Span, byte.MaxValue - 1));
            Assert.ThrowsException<FormatException>(() => EpicChain.IO.Helper.DecompressLz4(byteArray.Span, -1));

            // Error length

            byte[] data_wrong = byteArray.ToArray();
            data_wrong[0]++;
            Assert.ThrowsException<FormatException>(() => EpicChain.IO.Helper.DecompressLz4(data_wrong, byte.MaxValue));
        }

        [TestMethod]
        public void TestAsSerializableArray()
        {
            byte[] byteArray = EpicChain.IO.Helper.ToByteArray(new UInt160[] { UInt160.Zero });
            UInt160[] result = EpicChain.IO.Helper.AsSerializableArray<UInt160>(byteArray);
        ...
        public void TestGetVarSizeInt()
        {
            for (int i = 0; i < 3; i++)
            {
                if (i == 0)
                {
                    int result = EpicChain.IO.Helper.GetVarSize(1);
                    Assert.AreEqual(1, result);
                }
                else if (i == 1)
                {
                    int result = EpicChain.IO.Helper.GetVarSize(0xFFFF);
                    Assert.AreEqual(3, result);
                }
                else
                {
                    int result = EpicChain.IO.Helper.GetVarSize(0xFFFFFF);
        ...
        public void TestGetVarSizeGeneric()
        {
            for (int i = 0; i < 9; i++)
            {
                if (i == 0)
                {
                    int result = EpicChain.IO.Helper.GetVarSize(new UInt160[] { UInt160.Zero });
                ...
                else if (i == 1)//sbyte
                {
                    ...
                    int result = EpicChain.IO.Helper.GetVarSize(testList);
                ...
                else if (i == 8)
                {
                    ...
                    int result = EpicChain.IO.Helper.GetVarSize<int>(testList);
                    Assert.AreEqual(5, result);
                }
            }
        }
        ...
        {
            int result = EpicChain.IO.Helper.GetVarSize("AA");
            Assert.AreEqual(3, result);
        }
        ...
        public void TestReadSerializable()
        {
            ...
            EpicChain.IO.Helper.Write(writer, UInt160.Zero);
            MemoryReader reader = new(stream.ToArray());
            UInt160 result = EpicChain.IO.Helper.ReadSerializable<UInt160>(ref reader);
            Assert.AreEqual(UInt160.Zero, result);
        }
        ...
        public void TestReadSerializableArray()
        {
            MemoryStream stream = new();
            BinaryWriter writer = new(stream);
            EpicChain.IO.Helper.Write(writer, new UInt160[] { UInt160.Zero });
            MemoryReader reader = new(stream.ToArray());
            UInt160[] resultArray = EpicChain.IO.Helper.ReadSerializableArray<UInt160>(ref reader);
        ...
        public void TestReadVarBytes()
        {
            MemoryStream stream = new();
            BinaryWriter writer = new(stream);
            EpicChain.IO.Helper.WriteVarBytes(writer, new byte[] { 0xAA, 0xAA });
            stream.Seek(0, SeekOrigin.Begin);
            BinaryReader reader = new(stream);
            byte[] byteArray = EpicChain.IO.Helper.ReadVarBytes(reader, 10);
        ...
        public void TestReadVarInt()
        {
            for (int i = 0; i < 4; i++)
            {
                if (i == 0)
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    EpicChain.IO.Helper.WriteVarInt(writer, 0xFFFF);
                    stream.Seek(0, SeekOrigin.Begin);
                    BinaryReader reader = new(stream);
                    ulong result = EpicChain.IO.Helper.ReadVarInt(reader, 0xFFFF);
                ...
                else if (i == 1)
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    EpicChain.IO.Helper.WriteVarInt(writer, 0xFFFFFFFF);
                    stream.Seek(0, SeekOrigin.Begin);
                    BinaryReader reader = new(stream);
                    ulong result = EpicChain.IO.Helper.ReadVarInt(reader, 0xFFFFFFFF);
                ...
                else
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    EpicChain.IO.Helper.WriteVarInt(writer, 0xFFFFFFFFFF);
                    stream.Seek(0, SeekOrigin.Begin);
                    BinaryReader reader = new(stream);
                    Action action = () => EpicChain.IO.Helper.ReadVarInt(reader, 0xFFFFFFFF);
        ...
        {
            byte[] byteArray = EpicChain.IO.Helper.ToArray(UInt160.Zero);
        ...
        public void TestWriteFixedString()
        {
            for (int i = 0; i < 5; i++)
            {
                if (i == 0)
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    Action action = () => EpicChain.IO.Helper.WriteFixedString(writer, null, 0);
                ...
                else if (i == 1)
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    Action action = () => EpicChain.IO.Helper.WriteFixedString(writer, "AA", Encoding.UTF8.GetBytes("AA").Length - 1);
                ...
                else if (i == 2)
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    Action action = () => EpicChain.IO.Helper.WriteFixedString(writer, "拉拉", Encoding.UTF8.GetBytes("拉拉").Length - 1);
                    action.Should().Throw<ArgumentException>();
                }
                else if (i == 3)
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    EpicChain.IO.Helper.WriteFixedString(writer, "AA", Encoding.UTF8.GetBytes("AA").Length + 1);
                    stream.Seek(0, SeekOrigin.Begin);
                    byte[] byteArray = new byte[stream.Length];
                    stream.Read(byteArray, 0, (int)stream.Length);
        ...
        public void TestWriteVarBytes()
        {
            MemoryStream stream = new();
            BinaryWriter writer = new(stream);
            EpicChain.IO.Helper.WriteVarBytes(writer, new byte[] { 0xAA });
            stream.Seek(0, SeekOrigin.Begin);
            byte[] byteArray = new byte[stream.Length];
            stream.Read(byteArray, 0, (int)stream.Length);
        ...
        public void TestWriteVarInt()
        {
            for (int i = 0; i < 5; i++)
            {
                if (i == 0)
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    Action action = () => EpicChain.IO.Helper.WriteVarInt(writer, -1);
                ...
                else if (i == 1)
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    EpicChain.IO.Helper.WriteVarInt(writer, 0xFC);
                    stream.Seek(0, SeekOrigin.Begin);
                    byte[] byteArray = new byte[stream.Length];
                    stream.Read(byteArray, 0, (int)stream.Length);
                ...
                else
                {
                    MemoryStream stream = new();
                    BinaryWriter writer = new(stream);
                    EpicChain.IO.Helper.WriteVarInt(writer, 0xAEFFFFFFFF);
                    stream.Seek(0, SeekOrigin.Begin);
                    byte[] byteArray = new byte[stream.Length];
                    stream.Read(byteArray, 0, (int)stream.Length);
        ...
        public void TestWriteVarString()
        {
            MemoryStream stream = new();
            BinaryWriter writer = new(stream);
            EpicChain.IO.Helper.WriteVarString(writer, "a");
            stream.Seek(0, SeekOrigin.Begin);
            byte[] byteArray = new byte[stream.Length];
            stream.Read(byteArray, 0, (int)stream.Length);
