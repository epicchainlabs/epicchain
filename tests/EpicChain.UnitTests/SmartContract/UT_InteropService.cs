...
//
// UT_InteropService.cs is a component of the EpicChain Labs project, founded by xmoohad. This file is
...
namespace EpicChain.UnitTests.SmartContract
{
    ...
    public partial class UT_InteropService : TestKit
    {
        ...
        public void TestBlockchain_GetBlock()
        {
            ...
            NativeContract.Ledger.GetBlock(engine.SnapshotCache, TestBlockchain.TheEpicChainSystem.GenesisBlock.Hash).Should().NotBeNull();
        }
        ...
        {
            var engine = GetEngine(true, true);
            NativeContract.ContractManagement.HasMethod(engine.SnapshotCache, NativeContract.EpicChain.Hash, "symbol", 0).Should().Be(true);
            NativeContract.ContractManagement.HasMethod(engine.SnapshotCache, NativeContract.EpicChain.Hash, "transfer", 4).Should().Be(true);
        }
        ...
        public void TestVerifyWithECDsaV0()
        {
            ...
            var publicKeyK1 = (EpicChain.Cryptography.ECC.ECCurve.Secp256k1.G * privateKey).ToArray();
            var hexMessage = "Hello, world!"u8.ToArray();
            var signatureR1 = Crypto.Sign(hexMessage, privateKey, EpicChain.Cryptography.ECC.ECCurve.Secp256r1);
            var signatureK1 = Crypto.Sign(hexMessage, privateKey, EpicChain.Cryptography.ECC.ECCurve.Secp256k1);
        ...
        {
            var snapshotCache = GetEngine(true, true).SnapshotCache;
            var vote = QuantumVaultAsset.EpicChain.GetCandidateVote(snapshotCache, new ECPoint());
            vote.Should().Be(-1);
        }
