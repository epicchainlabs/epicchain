...
//
// UT_EpicChain.cs is a component of the EpicChain Labs project, founded by xmoohad. This file is
...
namespace EpicChain.UnitTests.SmartContract.Native
{
    ...
    public class UT_EpicChain
    {
        ...
        public void Check_RegisterValidator()
        {
            ...
            var members = NativeContract.EpicChain.GetCandidatesInternal(clonedCache);
        ...
        public void Check_UnregisterCandidate()
        {
            ...
            members = NativeContract.EpicChain.GetCandidatesInternal(clonedCache);
        ...
        public void Check_GetCommittee()
        {
            ...
            var committeemembers = NativeContract.EpicChain.GetCommittee(clonedCache);
            ...
            committeemembers = NativeContract.EpicChain.GetCommittee(clonedCache);
        ...
        public void Check_Transfer()
        {
            ...
            NativeContract.EpicChain.Transfer(clonedCache, from, to, BigInteger.One, false, persistingBlock).Should().BeFalse(); // Not signed
            NativeContract.EpicChain.Transfer(clonedCache, from, to, BigInteger.One, true, persistingBlock).Should().BeTrue();
            NativeContract.EpicChain.BalanceOf(clonedCache, from).Should().Be(99999999);
            NativeContract.EpicChain.BalanceOf(clonedCache, to).Should().Be(1);
            ...
            Assert.ThrowsException<ArgumentOutOfRangeException>(() => NativeContract.EpicChain.Transfer(clonedCache, from, to, BigInteger.MinusOne, true, persistingBlock));
            Assert.ThrowsException<FormatException>(() => NativeContract.EpicChain.Transfer(clonedCache, new byte[19], to, BigInteger.One, false, persistingBlock));
            Assert.ThrowsException<FormatException>(() => NativeContract.EpicChain.Transfer(clonedCache, from, new byte[19], BigInteger.One, false, persistingBlock));

            // More than balance

            NativeContract.EpicChain.Transfer(clonedCache, to, from, new BigInteger(2), true, persistingBlock).Should().BeFalse();
        }
        ...
        public void Check_BalanceOf()
        {
            ...
            NativeContract.EpicChain.BalanceOf(clonedCache, account).Should().Be(100_000_000);

            account[5]++; // Without existing balance

            NativeContract.EpicChain.BalanceOf(clonedCache, account).Should().Be(0);
        }
        ...
        public void TestCalculateBonus()
        {
            ...
            Action action = () => NativeContract.EpicChain.UnclaimedEpicPulse(clonedCache, UInt160.Zero, 10).Should().Be(new BigInteger(0));
            ...
            action = () => NativeContract.EpicChain.UnclaimedEpicPulse(clonedCache, UInt160.Zero, 10).Should().Be(new BigInteger(0));
            clonedCache.Delete(key);
            ...
            NativeContract.EpicChain.UnclaimedEpicPulse(clonedCache, UInt160.Zero, 100).Should().Be(new BigInteger(0.5 * 100 * 100));
            clonedCache.Delete(key);
            ...
            clonedCache.Add(new KeyBuilder(NativeContract.EpicChain.Id, 23).Add(TestProtocolSettings.Default.StandbyCommittee[0]).AddBigEndian(uint.MaxValue - 50), new StorageItem() { Value = new BigInteger(50 * 10000L).ToByteArray() });
            NativeContract.EpicChain.UnclaimedEpicPulse(clonedCache, UInt160.Zero, 100).Should().Be(new BigInteger(50 * 100));
            clonedCache.Delete(key);
        }
        ...
        public void TestGetNextBlockValidators1()
        {
            ...
            var result = (VM.Types.Array)NativeContract.EpicChain.Call(snapshotCache, "getNextBlockValidators");
        ...
        {
            var clonedCache = _snapshotCache.CloneCache();
            var result = NativeContract.EpicChain.GetNextBlockValidators(clonedCache, 7);
        ...
        {
            var snapshotCache = TestBlockchain.GetTestSnapshotCache();
            var array = (VM.Types.Array)NativeContract.EpicChain.Call(snapshotCache, "getCandidates");
        ...
        {
            var clonedCache = _snapshotCache.CloneCache();
            var result = NativeContract.EpicChain.GetCandidatesInternal(clonedCache);
            result.Count().Should().Be(0);

            StorageKey key = NativeContract.EpicChain.CreateStorageKey(33, ECCurve.Secp256r1.G);
            clonedCache.Add(key, new StorageItem(new CandidateState() { Registered = true }));
            NativeContract.EpicChain.GetCandidatesInternal(clonedCache).Count().Should().Be(1);
        }
        ...
        public void TestCheckCandidate()
        {
            ...
            var committee = NativeContract.EpicChain.GetCommittee(cloneCache);
            var point = committee[0].EncodePoint(true);

            // Prepare Prefix_VoterRewardPerCommittee
            var storageKey = new KeyBuilder(NativeContract.EpicChain.Id, 23).Add(committee[0]);
            cloneCache.Add(storageKey, new StorageItem(new BigInteger(1000)));

            // Prepare Candidate
            storageKey = new KeyBuilder(NativeContract.EpicChain.Id, 33).Add(committee[0]);
            cloneCache.Add(storageKey, new StorageItem(new CandidateState { Registered = true, Votes = BigInteger.One }));

            storageKey = new KeyBuilder(NativeContract.EpicChain.Id, 23).Add(committee[0]);
        ...
        public void TestGetCommittee()
        {
            ...
            var result = (VM.Types.Array)NativeContract.EpicChain.Call(clonedCache, "getCommittee");
        ...
        {
            var clonedCache = _snapshotCache.CloneCache();
            var result = NativeContract.EpicChain.ComputeNextBlockValidators(clonedCache, TestProtocolSettings.Default);
            result[0].ToArray().ToHexString().Should().Be("02486fd15702c4490a26703112a5cc1d0923fd697a33406bd5a1c00e0013b09a70");
            result[1].ToArray().ToHexString().Should().Be("024c7b7fb6c310fccf1ba33b082519d82964ea93868d676662d4a59ad548df0e7d");
            result[2].ToArray().ToHexString().Should().Be("02aaec38470f6aad0042c6e877cfd8087d2676b0f516fddd362801b9bd3936399e");
            result[3].ToArray().ToHexString().Should().Be("03b209fd4f53a7170ea4444e0cb0a6bb6a53c2bd016926989cf85f9b0fba17a70c");
        ...
        {
            var clonedCache = _snapshotCache.CloneCache();
            NativeContract.EpicChain.TotalSupply(clonedCache).Should().Be(new BigInteger(100000000));
        }
        ...
        public void TestEconomicParameter()
        {
            ...
            height.Index = persistingBlock.Index + 1;
            NativeContract.EpicChain.UnclaimedEpicPulse(clonedCache, UInt160.Zero, persistingBlock.Index + 2).Should().Be(6500);
        }
        ...
        public void TestClaimEpicPulse()
        {
            ...
            for (var i = 0; i < TestProtocolSettings.Default.CommitteeMembersCount; i++)
            {
                ...
                clonedCache.Add(new KeyBuilder(NativeContract.EpicChain.Id, 33).Add(member), new StorageItem(new CandidateState()
                {
                    Registered = true,
                    Votes = 200 * 10000
                }));
            ...
            clonedCache.GetOrAdd(new KeyBuilder(NativeContract.EpicChain.Id, 14), () => new StorageItem()).Value = BinarySerializer.Serialize(cachedCommittee.ToStackItem(null), ExecutionEngineLimits.Default);

            var item = clonedCache.GetAndChange(new KeyBuilder(NativeContract.EpicChain.Id, 1), () => new StorageItem());
            ...
            NativeContract.EpicChain.BalanceOf(clonedCache, Contract.CreateSignatureContract(accountA).ScriptHash).Should().Be(0);

            StorageItem storageItem = clonedCache.TryGet(new KeyBuilder(NativeContract.EpicChain.Id, 23).Add(accountA));
            ((BigInteger)storageItem).Should().Be(30000000000);

            clonedCache.TryGet(new KeyBuilder(NativeContract.EpicChain.Id, 23).Add(accountB).AddBigEndian(uint.MaxValue - 1)).Should().BeNull();
            ...
            NativeContract.EpicChain.BalanceOf(clonedCache, Contract.CreateSignatureContract(committee[1]).ScriptHash).Should().Be(0);

            storageItem = clonedCache.TryGet(new KeyBuilder(NativeContract.EpicChain.Id, 23).Add(committee[1]));
            ((BigInteger)storageItem).Should().Be(30000000000);
            ...
            NativeContract.EpicChain.BalanceOf(clonedCache, Contract.CreateSignatureContract(committee[2]).ScriptHash).Should().Be(0);

            storageItem = clonedCache.TryGet(new KeyBuilder(NativeContract.EpicChain.Id, 23).Add(committee[2]));
            ((BigInteger)storageItem).Should().Be(30000000000 * 2);

            // Claim EpicPulse

            var account = Contract.CreateSignatureContract(committee[2]).ScriptHash;
            clonedCache.Add(new KeyBuilder(NativeContract.EpicChain.Id, 20).Add(account), new StorageItem(new EpicChainAccountState
            {
                BalanceHeight = 3,
            ...
            NativeContract.EpicChain.BalanceOf(clonedCache, account).Should().Be(1999800);
            var storageKey = new KeyBuilder(NativeContract.Ledger.Id, 12);
            clonedCache.GetAndChange(storageKey).GetInteroperable<HashIndexState>().Index = 29 + 2;
            BigInteger value = NativeContract.EpicChain.UnclaimedEpicPulse(clonedCache, account, 29 + 3);
        ...
        {
            var clonedCache = _snapshotCache.CloneCache();
            NativeContract.EpicChain.UnclaimedEpicPulse(clonedCache, UInt160.Zero, 10).Should().Be(new BigInteger(0));
        ...
        internal (bool State, bool Result) Transfer4TesingOnBalanceChanging(BigInteger amount, bool addVotes)
        {
            var clonedCache = _snapshotCache.CloneCache();
            _persistingBlock.Header.Index = 1;
            var engine = ApplicationEngine.Create(TriggerType.Application, TestBlockchain.TheEpicChainSystem.GenesisBlock, clonedCache, _persistingBlock, settings: TestBlockchain.TheEpicChainSystem.Settings);
            ...
            if (addVotes)
            {
                ...
                clonedCache.Add(NativeContract.EpicChain.CreateStorageKey(33, ECCurve.Secp256r1.G), new StorageItem(new CandidateState()));
            }
            else
            {
                clonedCache.Add(CreateStorageKey(20, from.ToArray()), new StorageItem(new EpicChainAccountState
            ...
            sb.EmitDynamicCall(NativeContract.EpicChain.Hash, "transfer", from, UInt160.Zero, amount, null);
        ...
        internal static bool Check_OnPersist(DataCache clonedCache, Block persistingBlock)
        {
            ...
            var engine = ApplicationEngine.Create(TriggerType.OnPersist, null, clonedCache, persistingBlock, settings: TestBlockchain.TheEpicChainSystem.Settings);
        ...
        internal static bool Check_PostPersist(DataCache clonedCache, Block persistingBlock)
        {
            ...
            using var engine = ApplicationEngine.Create(TriggerType.PostPersist, null, clonedCache, persistingBlock, settings: TestBlockchain.TheEpicChainSystem.Settings);
            engine.LoadScript(script.ToArray());
        ...
        internal static (BigInteger Value, bool State) Check_GetepicpulsePerBlock(DataCache clonedCache, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.EpicChain.Hash, "getepicpulsePerBlock");
            engine.LoadScript(script.ToArray());
        ...
        {
            UInt160 committeeMultiSigAddr = NativeContract.EpicChain.GetCommitteeAddress(clonedCache);
            ...
            var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.EpicChain.Hash, "setepicpulsePerBlock", epicpulsePerBlock);
            engine.LoadScript(script.ToArray());
        ...
        internal static (bool State, bool Result) Check_Vote(DataCache clonedCache, byte[] account, byte[] pubkey, bool signAccount, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.EpicChain.Hash, "vote", account, pubkey);
            engine.LoadScript(script.ToArray());
        ...
        internal static (bool State, bool Result) Check_RegisterValidator(DataCache clonedCache, byte[] pubkey, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.EpicChain.Hash, "registerCandidate", pubkey);
            engine.LoadScript(script.ToArray());
        ...
        internal static ECPoint[] Check_GetCommittee(DataCache clonedCache, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.EpicChain.Hash, "getCommittee");
            engine.LoadScript(script.ToArray());
        ...
        internal static (BigInteger Value, bool State) Check_UnclaimedEpicPulse(DataCache clonedCache, byte[] address, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.EpicChain.Hash, "UnclaimedEpicPulse", address, persistingBlock.Index);
            engine.LoadScript(script.ToArray());
        ...
        internal static (bool State, bool Result) Check_UnregisterCandidate(DataCache clonedCache, byte[] pubkey, Block persistingBlock)
        {
            ...
            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.EpicChain.Hash, "unregisterCandidate", pubkey);
            engine.LoadScript(script.ToArray());
        ...
        {
            using var engine = ApplicationEngine.Create(TriggerType.Application, null, clonedCache, settings: TestBlockchain.TheEpicChainSystem.Settings);

            using var script = new ScriptBuilder();
            script.EmitDynamicCall(NativeContract.EpicChain.Hash, "getAccountState", account);
            engine.LoadScript(script.ToArray());
