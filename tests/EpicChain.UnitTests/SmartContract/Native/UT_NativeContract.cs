...
//
// UT_NativeContract.cs is a component of the EpicChain Labs project, founded by xmoohad. This file is
...
namespace EpicChain.UnitTests.SmartContract.Native
{
    ...
    public class UT_NativeContract
    {
        ...
        public void TestIsInitializeBlock()
        {
            ...
            Assert.IsTrue(NativeContract.CryptoHive.IsInitializeBlock(settings, 0, out var hf));
            Assert.IsNotNull(hf);
            Assert.AreEqual(0, hf.Length);

            Assert.IsFalse(NativeContract.CryptoHive.IsInitializeBlock(settings, 1, out hf));
            Assert.IsNull(hf);

            Assert.IsTrue(NativeContract.CryptoHive.IsInitializeBlock(settings, 20, out hf));
        ...
        public void TestGenesisXep17Manifest()
        {
            ...
            foreach (var h in new List<UInt160>() { NativeContract.EpicPulse.Hash, NativeContract.EpicChain.Hash })
            {
                var state = Call_GetContract(snapshot, h, persistingBlock);
                Assert.IsTrue(state.Manifest.SupportedStandards.Contains("XEP-17"));
        ...
        {
            using var engine = ApplicationEngine.Create(TriggerType.Application, null, snapshot, persistingBlock, settings: TestBlockchain.TheEpicChainSystem.Settings);
