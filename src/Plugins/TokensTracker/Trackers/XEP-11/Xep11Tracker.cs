...
//
// Xep11Tracker.cs is a component of the EpicChain Labs project, founded by xmoohad. This file is
        ...
        public override void OnPersist(EpicChainSystem system, Block block, DataCache snapshot, IReadOnlyList<Blockchain.ApplicationExecuted> applicationExecutedList)
        {
            ...
            foreach (Blockchain.ApplicationExecuted appExecuted in applicationExecutedList)
            {
                ...
                foreach (var notifyEventArgs in appExecuted.Notifications)
                {
                    ...
                    if (contract?.Manifest.SupportedStandards.Contains("XEP-11") == true)
                    {
                        try
                        {
